...
def main():
    ...
    #引数に与えたパスのbertファイル(それはpretrainされた学習ファイル)を与えて、トーカナイザーとして、判断基準に登録する

    # Pre-processing.
    x_train, x_test, y_train, y_test = train_test_split(x, y, test_size=0.2, random_state=42)
    target_vocab = Vocab().fit(y_train)
    features_train, labels_train = convert_examples_to_features(x_train, y_train, target_vocab, max_seq_length=maxlen, tokenizer=tokenizer)
    ...
    # Train the model.
    model.fit(x=features_train, y=labels_train, batch_size=batch_size, epochs=epochs, validation_split=0.1, callbacks=callbacks)
